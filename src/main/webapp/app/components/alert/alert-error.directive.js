(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('bookViewerApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-bookViewerApp-error');
                var entityKey = httpResponse.headers('X-bookViewerApp-params');
                if (errorHeader) {
